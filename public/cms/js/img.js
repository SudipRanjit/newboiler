...
function listMedia(page = 1) {
  ...
  $(".select").hide();
  var token = $("meta[name='csrf-token']").attr("content");
  var url = window.location.origin + "/" + "admin/allMedia?page="+page;

  var search = $("#img-search").val();

  if(search != "" && search != undefined)
    url = window.location.origin + "/" + "admin/allMedia/"+search+"?page="+page;
...
function updateMedia($media) {
  ...
  $.ajax({
    url: window.location.origin + "/" + "admin/updateMedia/" + $media,
    method: "POST",
    data: {
      id: $media,
      caption: $("#caption").val(),
...
function deleteMedia($id) {
  ...
  if (confirm("Are you sure?")) {
    $.ajax({
      url: window.location.origin + "/" + "admin/deleteMedia/" + $id,
      type: "DELETE",
      data: {
        id: $id,
        _token: token,
