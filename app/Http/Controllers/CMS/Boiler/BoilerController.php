...
use App\Webifi\Repositories\Boiler\TagRepository;
...
class BoilerController extends Controller
{
    ...
    public function store(BoilerRequest $request)
    {
        ...
        try {
            ...
            $tags = $this->tag->all()->pluck('tag', 'id')->toArray();
            $fTags = [];
            if(!empty($request->tags))
            {
              foreach($request->tags as $tag)
              {
                  if(!in_array(ucwords(strtolower($tag)), $tags) && !is_numeric($tag))
                  {
                    $tId = $this->tag->store(['tag' => ucwords(strtolower($tag))]);
                    $fTags[] = $tId->id;
                  }
                  if(is_numeric($tag))
                  {
                    ...
                    $fTags[] = $this->tag->findBy('tag', $tag)->id;
                  }
              }
            }
